...
WC = wordcloud.WordCloud(font_path='C:\\Windows\\Fonts\\STFANGSO.TTF', max_words=100, mask=MASK,
...
for a in range(len(fulltext_list)):
    ...
    paragraph_vecs[a] = get_sent_vec(paragraph_sent)[
        paragraph_sent[0]].tolist()
...
for a in range(len(fulltext_list)):
    for key in Vsj:
        sorted_paragraph = sorted(
            paragraph_dist[a].items(), key=lambda d: d[1], reverse=True)
